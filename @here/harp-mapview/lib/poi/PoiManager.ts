...
export class PoiManager {
    ...
    private static notifyMissingPoiTable(poiTableName: string,
        poiTable: PoiTable | undefined): void {
        ...
        if (PoiManager.m_missingPoiTableName.get(poiTableName) === undefined) {
            ...
            if (poiTable !== undefined && !poiTable.loadedOk) {
                logger.error(
                    `updatePoiFromPoiTable: Could not load POI table '${poiTableName}'!`);
            } else {
                logger.error(
    ...
    private checkCreateTextElement(
        ...
        if (text !== undefined) {
            ...
            if (imageTextureName === undefined && poiTableName !== undefined) {
                imageTextureName = `<lookup:  ${poiTableName}[${poiName}]>`;
            }
